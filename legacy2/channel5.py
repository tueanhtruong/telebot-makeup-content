...
def parse_channel_id(raw: str) -> Optional[int]:
	...
	try:
		...
		logger.warning("Invalid TELEGRAM_CHANNEL_5_ID: %s", raw)
		return None
...
channel_username = os.getenv("TELEGRAM_CHANNEL_5_USERNAME", "").strip()
channel_id = parse_channel_id(os.getenv("TELEGRAM_CHANNEL_5_ID", "").strip())
...
async def main() -> None:
	...
	if not channel_usernames and not channel_ids:
		raise ValueError("Set TELEGRAM_CHANNEL_5_USERNAME or TELEGRAM_CHANNEL_5_ID")
