...
def parse_channel_id(raw: str) -> Optional[int]:
	...
	try:
		...
		logger.warning("Invalid TELEGRAM_CHANNEL_7_ID: %s", raw)
		return None
...
channel_username = os.getenv("TELEGRAM_CHANNEL_7_USERNAME", "").strip()
channel_id = parse_channel_id(os.getenv("TELEGRAM_CHANNEL_7_ID", "").strip())

window_seconds = int(os.getenv("TELEGRAM_WINDOW_SECONDS", "600"))
fetch_limit = int(os.getenv("TELEGRAM_FETCH_LIMIT", "10"))
content_filter = os.getenv("TELEGRAM_CONTENT_FILTER", "media").strip().lower() or "media"
...
async def main() -> None:
	...
	if not channel_usernames and not channel_ids:
		raise ValueError("Set TELEGRAM_CHANNEL_7_USERNAME or TELEGRAM_CHANNEL_7_ID")
